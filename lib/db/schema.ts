...
export const extractedImages = sqliteTable(
  "extracted_images",
  (table) => {
    return {
      id: text("id")
...
export const indexingMetrics = sqliteTable(
  "indexing_metrics",
  (table) => {
    return {
      id: text("id")
...
export const ablationRuns = sqliteTable(
  "ablation_runs",
  {
    id: text("id")
      .primaryKey()
    ...
    questionId: text("question_id")
      .notNull()
      .references(() => ablationQuestions.id),
    configId: text("config_id")
      .notNull()
      .references(() => ablationConfigurations.id),
  ...
  (table) => ({
    uniqueRunPerBatch: index("unique_run_per_batch").on(
      table.questionId,
      table.configId,
      table.runBatch,
    ),
...
export const ablationScores = sqliteTable(
  "ablation_scores",
  {
    id: text("id")
      .primaryKey()
...
export const ablationRunChunks = sqliteTable(
  "ablation_run_chunks",
  {
    id: text("id")
      .primaryKey()
...
export const ablationSummary = sqliteTable(
  "ablation_summary",
  {
    id: text("id")
      .primaryKey()
  ...
  (table) => ({
    uniqueSummaryBatch: index("unique_summary_batch").on(
      table.runBatch,
      table.configId,
...
export const ablationScenarioTurns = sqliteTable(
  "ablation_scenario_turns",
  {
    id: text("id").primaryKey(),
    scenarioId: text("scenario_id")
      .notNull()
      .references(() => ablationScenarios.id),
...
export const ablationScenarioRuns = sqliteTable(
  "ablation_scenario_runs",
  {
    id: text("id").primaryKey(),
    scenarioId: text("scenario_id")
      .notNull()
  ...
  (table) => ({
    uniqueRun: uniqueIndex("idx_scen_runs_unique").on(
      table.scenarioId,
      table.configId,
      table.runBatch,
    ),
...
export const ablationScenarioTurnResults = sqliteTable(
  "ablation_scenario_turn_results",
  {
    id: text("id").primaryKey(),
    scenarioRunId: text("scenario_run_id")
      .notNull()
      .references(() => ablationScenarioRuns.id),
    scenarioTurnId: text("scenario_turn_id")
      .notNull()
      .references(() => ablationScenarioTurns.id),
    turnNumber: integer("turn_number").notNull(),
...
export const ablationScenarioScores = sqliteTable(
  "ablation_scenario_scores",
  {
    id: text("id").primaryKey(),
    scenarioRunId: text("scenario_run_id")
      .notNull()
...
export const ablationScenarioSummary = sqliteTable(
  "ablation_scenario_summary",
  {
    id: text("id").primaryKey(),
    configId: text("config_id")
      .notNull()
  ...
  (table) => ({
    uniqueScenSummary: uniqueIndex("idx_scen_summary_unique").on(
      table.configId,
      table.scenarioCategory,
    ),
  }),
);
...
// 23. Perfiles de Juez
export const judgeProfiles = sqliteTable(
  "judge_profiles",
  (table) => {
    return {
      id: text("id")
...
export const judgeCases = sqliteTable(
  "judge_cases",
  (table) => {
    return {
      id: text("id")
        .primaryKey()
        .$defaultFn(() => createId()),
      judgeProfileId: text("judge_profile_id")
        .notNull()
  ...
  (table) => ({
    ...
    uniqueCaseNumber: uniqueIndex("idx_judge_cases_unique_number").on(
      table.judgeProfileId,
      table.caseNumber,
    ),
  }),
);
  ...
  (table) => {
    return {
      ...
      judgeProfileId: text("judge_profile_id")
        .notNull()
        .references(() => judgeProfiles.id, { onDelete: "cascade" }),
      judgeCaseId: text("judge_case_id")
        .notNull()
        .references(() => judgeCases.id, { onDelete: "cascade" }),
...
export const judgeMessages = sqliteTable(
  "judge_messages",
  (table) => {
    return {
      id: text("id")
      ...
      sessionId: text("session_id")
        .notNull()
        .references(() => judgeSessions.id, { onDelete: "cascade" }),
...
// Juicio de Expertos
export type Expert = typeof expertos.$inferSelect;
export type NewExpert = typeof expertos.$inferInsert;
...
export type MiniEncuesta = typeof miniEncuestas.$inferSelect;
export type NewMiniEncuesta = typeof miniEncuestas.$inferInsert;
export type EncuestaFase1 = typeof encuestasFase1.$inferSelect;
export type NewEncuestaFase1 = typeof encuestasFase1.$inferInsert;
export type EncuestaFase2 = typeof encuestasFase2.$inferSelect;
export type NewEncuestaFase2 = typeof encuestasFase2.$inferInsert;
export type EncuestaFase3 = typeof encuestasFase3.$inferSelect;
export type NewEncuestaFase3 = typeof encuestasFase3.$inferInsert;
...
// ─── Juicio de Expertos (Nuevo Dashboard) ────────────────────────────────────

export const expertos = sqliteTable("expertos", (table) => {
  return {
    id: integer("id").primaryKey({ autoIncrement: true }),
    codigo: text("codigo").unique().notNull(),
    nombre: text("nombre").notNull(),
    empresa: text("empresa"),
    anosExperiencia: integer("anos_experiencia").notNull(),
    marcasDomina: text("marcas_domina"), // JSON
    certificaciones: text("certificaciones"), // JSON
    rolActual: text("rol_actual", {
      enum: ["tecnico_especialista", "comercial"],
    }).notNull(),
    zonaTrabajo: text("zona_trabajo"),
    contacto: text("contacto"),
    fechaRegistro: text("fecha_registro").default(sql`CURRENT_TIMESTAMP`),
  };
});

export const sesiones = sqliteTable("sesiones", (table) => {
  return {
    id: integer("id").primaryKey({ autoIncrement: true }),
    expertoId: integer("experto_id")
      .notNull()
      .references(() => expertos.id),
    fase: text("fase", { enum: ["fase1_categoria", "fase2_libre", "fase3_comercial"] }).notNull(),
    categoriaId: text("categoria_id"),
    modelo: text("modelo"),
    tipoActividad: text("tipo_actividad", { enum: ["sesion", "pregunta_individual"] }),
    numeroActividad: integer("numero_actividad"),
    fechaInicio: text("fecha_inicio").notNull(),
    fechaFin: text("fecha_fin"),
    duracionMinutos: real("duracion_minutos"),
    estado: text("estado", { enum: ["completada", "abandonada"] }).default("completada"),
  };
});

export const mensajes = sqliteTable("mensajes", (table) => {
  return {
    id: integer("id").primaryKey({ autoIncrement: true }),
    ...
    emisor: text("emisor", { enum: ["experto", "sistema"] }).notNull(),
    contenido: text("contenido").notNull(),
    documentosCitados: text("documentos_citados"), // JSON
    tiempoRespuestaSeg: real("tiempo_respuesta_seg"),
  };
});

export const miniEncuestas = sqliteTable("mini_encuestas", (table) => ({
  id: integer("id").primaryKey({ autoIncrement: true }),
  sesionId: integer("sesion_id")
    .notNull()
    .references(() => sesiones.id, { onDelete: "cascade" }),
  utilidad: integer("utilidad").notNull(), // 1-5
  modo: text("modo"), // teorico | procedimental | diagnostico
}));

export const encuestasFase1 = sqliteTable("encuestas_fase1", (table) => {
  return {
    sesionId: integer("sesion_id")
      .primaryKey()
      .references(() => sesiones.id, { onDelete: "cascade" }),
    p1SistemaEntendio: integer("p1_sistema_entendio"),
    p2RespuestaCorrecta: integer("p2_respuesta_correcta"),
    p3FaltoPreguntar: integer("p3_falto_preguntar"),
    p4ClaridadJunior: integer("p4_claridad_junior"),
    p5UsariaManana: integer("p5_usaria_manana"), // 0=No, 1=Sí, 2=Dudas
    comentarioAdicional: text("comentario_adicional"),
    evaluacionesActividades: text("evaluaciones_actividades"), // JSON array of {numero, tipo, sesion_id, utilidad}
  };
});

export const encuestasFase2 = sqliteTable("encuestas_fase2", (table) => {
  return {
    sesionId: integer("sesion_id")
      .primaryKey()
      .references(() => sesiones.id, { onDelete: "cascade" }),
    p1ResolvioAcerco: integer("p1_resolvio_acerco"), // 0=No, 1=Parcial, 2=Sí
    p2MomentoFalla: integer("p2_momento_falla"),
    p3AlgoPeligroso: integer("p3_algo_peligroso"),
    p4ViabilidadJunior: integer("p4_viabilidad_junior"), // 0=No, 1=Parcial, 2=Sí
    p5Recomendaria: integer("p5_recomendaria"), // 0=No, 1=Parcial, 2=Sí
    comentarioAdicional: text("comentario_adicional"),
    evaluacionesActividades: text("evaluaciones_actividades"), // JSON array of {numero, tipo, sesion_id, utilidad}
  };
});

export const encuestasFase3 = sqliteTable("encuestas_fase3", (table) => {
  return {
    sesionId: integer("sesion_id")
      .primaryKey()
      .references(() => sesiones.id, { onDelete: "cascade" }),
    p1ComprendeProducto: text("p1_comprende_producto"),
    p2MercadoObjetivo: text("p2_mercado_objetivo"),
    p3ModeloCobro: text("p3_modelo_cobro"),
    p4FaltaParaVender: text("p4_falta_para_vender"),
    p5Compraria: integer("p5_compraria"),
    p6Recomendaria: integer("p6_recomendaria"),
    p7AbreServicio: integer("p7_abre_servicio"),
    comentarioAdicional: text("comentario_adicional"),
  };
});
